import FactoryMethodScene from "./FactoryMethodScene";
...
export default class FactoryMethodSceneRTL extends FactoryMethodScene {
    getColorBlocks(colorsNumber = 360, type = 'color') {
        function getColorSet(colorsNumber = 360) {
            let colors = [],
                H = 360,
                dec = Math.round(360 / colorsNumber);
            for (; H > 300; H -= dec) {
                let _r = 100,
                    _g = 0,
                    _b = 100 - (100 * ((H % 60) / 60));
        ...
        let colors = this.getColorSet(colorsNumber),
            colorBlocks = [],
