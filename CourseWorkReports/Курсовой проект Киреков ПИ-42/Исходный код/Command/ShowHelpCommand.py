...
class ShowHelpCommand(AbstractChainElement, ICommand):
    """ Команда вывода справочной информации """

    def __init__(self, state):
        self.__key = CommandsKeys.show_help()
        super().__init__(self.__key, None)
        assert isinstance(state, BaseState), \
            "Ошибка в ShowHelpCommand.__init__():\n" \
            "state должен наследовать/быть объектом класса BaseState"
        self.__state = state

    def execute(self):
        self.__state.to_current_state()
    ...
    def get_info(self):
        return {'command': self.__key, 'text': 'Показать список возможных команд'}